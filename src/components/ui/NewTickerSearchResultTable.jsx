...
import "./NewTickerSearchResultTable.css";

function NewTickerSerchResultTable({
  tickerTableData,
  searchText,
  setPortfolioChanged,
  ...
  const [assetLookup] = useState(
    portfolioDetailRecord?.summary?.assets.map((asset) => {
  ...
  const addSymbolClicked = (value) => {
    //Value objectStructure: key, companyName, ticker: symbol, quoteType, exchDisp
    const { ticker: symbol } = value;
    console.log("Add symbol clicked:", symbol, portfolioDetailRecord);

    PortfolioService.addAssetToPortfolio(
      portfolioDetailRecord.portfolioId,
      symbol
    )
      .then(
        (response) => {
...
export default NewTickerSerchResultTable;
